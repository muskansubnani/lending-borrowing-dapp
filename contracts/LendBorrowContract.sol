...
contract LendBorrowContract is Ownable, ReentrancyGuard {
    ...
    function createLoan (uint _loanAmount, uint _loanDuration, address _nftAddress, uint _nftTokenId, uint _nftFloorPrice
        ...
        require(IERC721(_nftAddress).ownerOf(_nftTokenId) == msg.sender, "Not owner of nft");

        require(_loanAmount <= _nftFloorPrice, 'Sorry, we cannot approve this loan for the given nft collateral');
